...
echo "===== [1/3] NORMALIZANDO yt-alemanes a 30fps ($(date +%H:%M)) ====="
cd public/assets/yt-alemanes
for f in marca-*.mp4; do
  case "$f" in *.cfr.mp4) continue;; esac
  o="${f%.mp4}.cfr.mp4"
  "$FF" -y -hide_banner -loglevel error -i "$f" -vf "fps=30" -c:v libx264 -preset veryfast -crf 22 -g 30 -keyint_min 30 -sc_threshold 0 -pix_fmt yuv420p -an -movflags +faststart "$o" && mv -f "$o" "$f" && echo "  OK $f"
done
cd "C:/Users/aleja/Desktop/car-channel-editor"
echo "===== [2/3] REGENERANDO config ($(date +%H:%M)) ====="
YT_SUBDIR=yt-alemanes node scripts/build-video.mjs 2>&1 | grep -iE "repetidos|Listo|ancla no" || exit 3
